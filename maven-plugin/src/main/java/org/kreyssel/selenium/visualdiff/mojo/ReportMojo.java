...
import org.kreyssel.selenium.visualdiff.mojo.diff.ImageCompare;
import org.kreyssel.selenium.visualdiff.mojo.diff.ImageCompare;

/**
 * ReportMojo generates a report of visual diffs between two selenium2 functional tests runs.
 * 
...
public class ReportMojo extends AbstractMavenReport {
	...
	protected void executeReport(final Locale arg0) throws MavenReportException {
		...
		try {
			currentArchiveFile = resolveScreenshotArtifact(artifact, project.getArtifact().getVersion());
		} catch (Exception ex) {
			throw new MavenReportException("Error on resolve screenshot artifact for current project!",ex);
		}
		
		if(currentArchiveFile==null){
			throw new MavenReportException("Could not found screenshot archive! Did you ensure that you run the package goal before?");
		}
		...
		try {
			...
			throw new MavenReportException("Error on resolve screenshot artifact for latest project!",ex);
		}
		
		if(previousArchiveFile==null){
			getLog().warn("Could not found a previous release version of artifact '"+project.getArtifact()+"'!");
			return;
		}
		...
		try {
			...
			throw new MavenReportException("Error on retrieving previous version screenshots list!",ex);
		}
		
		for(ScreenshotMeta currentScreenshot:currentScreenshots) {
			if(lastScreenshots.contains(currentScreenshot)) {
				try {
					ScreenshotMeta previousScreenshot = lastScreenshots.get(lastScreenshots.indexOf(currentScreenshot));
					
					InputStream inCurr = currentScreenshotsStore.getInputStream(currentScreenshot.path);
					InputStream inLast = previousScreenshotsStore.getInputStream(previousScreenshot.path);
					...
					if(!equal) {
						...
						getLog().info("Detect a different screenshot and stored diff image in '"+diffFile.getAbsolutePath()+"'!");
	...
	protected File getScreenshotsFromLatestRelease(Artifact artifact) throws ArtifactMetadataRetrievalException, ArtifactResolutionException, ArtifactNotFoundException {
		...
		ArtifactVersions artifactVersions = new ArtifactVersions(artifact,
					artifactMetadataSource.retrieveAvailableVersions(artifact, localRepository,
							remoteArtifactRepositories), VersionComparators.getVersionComparator("maven"));
	...
	protected File resolveScreenshotArtifact(Artifact artifact, String version) throws ArtifactResolutionException, ArtifactNotFoundException{
		
		// resolve screenshots.zip artifact
		Artifact resolveArtifact = artifactFactory.createArtifactWithClassifier(artifact.getGroupId(), artifact.getArtifactId(), version, "zip", "screenshots");
